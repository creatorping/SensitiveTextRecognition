...
def train_worker(rank, world_size, config):
    ...
    # Recreate dataloaders with distributed samplers (优化内存利用)
    train_loader = torch.utils.data.DataLoader(
        train_loader.dataset,
        batch_size=config.batch_size,
        sampler=train_sampler,
        num_workers=16,  # Increased from 8 to utilize more CPU/memory
        pin_memory=True,
        collate_fn=collate_fn,
        prefetch_factor=8,  # Increased from 4 for better prefetching
