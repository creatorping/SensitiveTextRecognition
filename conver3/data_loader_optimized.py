"""
Optimized Data Loader for H800 GPUs
针对H800优化的数据加载器，提升数据加载速度
"""
import torch
...
import numpy as np
from config_h800 import Config
...
class PrivacyDataset(Dataset):
    def __init__(self, data_path: str, label_path: str, tokenizer, config: Config, is_train=True):
        ...
        # Pre-compute tokenization for faster training (optional, uses more memory)
    ...
    def _load_data(self, data_path: str, label_path: str) -> Tuple[List[str], List[List[str]]]:
        ...
        assert len(texts) == len(labels), f"Text and label counts don't match: {len(texts)} vs {len(labels)}"
...
def create_dataloader(data_path: str, label_path: str, config: Config, is_train=True):
    """Create optimized dataloader for H800"""
    ...
    dataloader = DataLoader(
        ...
        drop_last=is_train  # 训练时丢弃最后不完整的batch
    )
